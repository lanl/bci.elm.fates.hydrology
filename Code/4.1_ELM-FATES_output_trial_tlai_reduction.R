...
current.folder <- "2019-10-14_5000/tlai_reduction"
...
col2 <- c("Observed" = "#f04546",  "Observed Gap-filled" = "purple", "Half HKsatADJ" = "blue", "Full BBslope" = "purple", "Half BBslope" = "green", "Half BBslope+ Half Vcmax" = "black")
...
par.on <- par.on.1[rep(row.names(par.on.1), 3),]
nsam <- length(par.on$par.sam) 
rmse.table <- data.frame(par.sam = par.on$par.sam, "qrunoff_daily" = rep(NA, nsam), n = c(7012, 7013, 7014))
rsq.table <- data.frame(par.sam = par.on$par.sam, "qrunoff_daily" = rep(NA, nsam), n = c(7012, 7013, 7014))
nse.table <- data.frame(par.sam = par.on$par.sam, "qrunoff_daily" = rep(NA, nsam), n = c(7012, 7013, 7014))

  

###
#### TLAI: Obs versus model #-------
####
# Monthly #------
####
###

obs.lai <- read.csv(file = file.path("/Users/rutuja/Work_at_LANL/Projects/bci.hydromet/data-raw/data1/lai_by_gap_fraction_matteo.csv"), header = TRUE)
head(obs.lai)
obs.lai <- obs.lai %>% transmute(date = as.Date(Date, format = "%d-%b-%y"),
                      obs = LAI)
load(file.path("data-raw/extract", current.folder, "extract/TLAI.h0.extract.Rdata"))
mod.lai.m <- setDT(as.data.frame(t(var.res.arr[[1]])), keep.rownames = "date") %>%
  mutate(date = as.Date(paste0(date, "-01")))
lai.m <- obs.lai %>% full_join(mod.lai.m, by = "date")
lai.m.long <- gather(lai.m, key = "par.sam", "value", -date, -obs) 
lai.m.long.sub <- lai.m.long %>% subset(date >= as.Date("2013-01-01")) %>%
  arrange(par.sam, date)

p.lai.m <- ggplot(lai.m.long.sub,
                   aes(x = date, y = value)) +
  geom_point(data = lai.m.long.sub %>% subset(par.sam == 7012), aes(group = par.sam, color = "Half BBslope"), show.legend = F, size = 0.5) +
  geom_point(data = lai.m.long.sub %>% subset(par.sam == 7013), aes(group = par.sam, color = "Full BBslope"), show.legend = F, size = 0.5) +
  geom_point(data = lai.m.long.sub %>% subset(par.sam == 7014), aes(group = par.sam, color = "Half BBslope + Half Vcmax"), show.legend = F, size = 0.5) +
  geom_point(aes(y = obs, colour = "Observed"), size = 0.5) +
  scale_colour_manual(name = "", values = col2) +
  ylab("TLAI [unitless]") +
  xlab("Date") + 
  theme(legend.text = element_text(size = 16, face = "plain"),
        legend.position = c(0.2, 0.5), legend.background = element_rect(fill = "transparent")) +
  scale_x_date(date_breaks = "1 year", labels = function(x) format(x, "%b%y")) +
  ggtitle("TLAI: Observed vs Simulated_Daily")
p.lai.m
ggsave("TLAI_Obs_vs_model_monthly.jpeg", plot = p.lai.m, path = file.path("figures", current.folder), device = "jpeg", height = 6.25, width = 8.94, units='in')
...
p.qrun.d <- ggplot(qrunoff.d.long.sub,
       aes(x = date, y = value)) +
  # geom_line(data = qrunoff.d.long.sub %>% subset(par.sam == 7015), aes(group = par.sam, color = "Half Vcmax"), show.legend = F, size = 0.5) +
  # geom_line(data = qrunoff.d.long.sub %>% subset(par.sam == 7011), aes(group = par.sam, color = "Half HKsatADJ"), show.legend = F, size = 0.5) +
  geom_line(data = qrunoff.d.long.sub %>% subset(par.sam == 7014), aes(group = par.sam, color = "Half BBslope+ Half Vcmax"), show.legend = F, size = 0.5) +
  geom_line(data = qrunoff.d.long.sub %>% subset(par.sam == 7013), aes(group = par.sam, color = "Full BBslope"), show.legend = F, size = 0.5) +
  geom_line(data = qrunoff.d.long.sub %>% subset(par.sam == 7012), aes(group = par.sam, color = "Half BBslope"), show.legend = F, size = 0.5) +
...
p.qrun.m <- ggplot(qrunoff.m.long.sub,
       aes(x = date, y = value)) +
  geom_line(data = qrunoff.m.long.sub %>% subset(par.sam == 7012), aes(group = par.sam, color = "leafage"), show.legend = F, size = 0.5) +
  geom_line(data = qrunoff.m.long.sub %>% subset(par.sam == 7013), aes(group = par.sam, color = "Half Vcmax"), show.legend = F, size = 0.5) +
  geom_line(data = qrunoff.m.long.sub %>% subset(par.sam == 7014), aes(group = par.sam, color = "leafage_fdrought"), show.legend = F, size = 0.5) +
  geom_line(aes(y = obs, colour = "Observed"), size = 0.5) +
...
qet.d.long.points <- qet.d.long %>% subset(date >= as.Date("2012-07-01") & date < as.Date("2017-08-31")) %>%droplevels()
...
p.et1 <- ggplot(qet.d.long.sub,
                aes(x = date, y = value)) +
  geom_point(aes(group = par.sam, color = "Simulated"), show.legend = F, size = 0.1) +
...
col1 <- c("Observed" = "#f04546", "Simulated" = "#3591d1", "Best-fit Simulated" = "green", 
          "Best-fit NSE" = "green", "Best-fit R-squared" = "purple", 
          "Observed at Ava-Tower" = "#f04546", "Observed Plot-wide" = "black")
p.et1.p <- ggplot(qet.d.long.points, aes(x = date, y = value)) +
  geom_path(aes(x= date, y = obs.2, color = "Observed"), size = 0.2, linetype = 5) +  
  ...
  xlab("Date") + 
  scale_x_date(date_breaks = "1 year", labels = function(x) format(x, "%b%y"))
 # geom_line(data = qet.d.long.points %>% subset(par.sam == 7015), aes(group = par.sam, color = "Half Vcmax"), show.legend = F, size = 0.5) +
 # geom_line(data = qet.d.long.points %>% subset(par.sam == 7011), aes(group = par.sam, color = "Half HKsatADJ"), show.legend = F, size = 0.5) +
  # geom_line(data = qet.d.long.points %>% subset(par.sam == 7013), aes(group = par.sam, color = "Full BBslope"), show.legend = F, size = 0.1) +
  # geom_line(data = qet.d.long.points %>% subset(par.sam == 7012), aes(group = par.sam, color = "Half BBslope"), show.legend = F, size = 0.1) +
  
p.et.p.halfbb.halfvc <-  p.et1.p +
  geom_line(data = qet.d.long.points %>% subset(par.sam == 7014), aes(group = par.sam, color = "Half BBslope+ Half Vcmax"), show.legend = F, size = 0.2)
ggsave(paste0("ET_Obs_vs_model_daily_all_years_points_lines_halfbb.halfvc.jpeg"), plot = p.et.p.halfbb.halfvc, path = file.path("figures", current.folder), device = "jpeg", height = 3, width = 15, units='in')

p.et1.d <- ggplot(qet.d.long.sma, aes(x = date, y = value)) +
  # geom_line(data = qet.d.long.sma %>% subset(par.sam == 7015), aes(group = par.sam, color = "Half Vcmax"), show.legend = F, size = 0.5) +
  # geom_line(data = qet.d.long.sma %>% subset(par.sam == 7011), aes(group = par.sam, color = "Half HKsatADJ"), show.legend = F, size = 0.5) +
  geom_line(data = qet.d.long.sma %>% subset(par.sam == 7014), aes(group = par.sam, color = "Half BBslope+ Half Vcmax"), show.legend = F, size = 0.5) +
  geom_line(data = qet.d.long.sma %>% subset(par.sam == 7013), aes(group = par.sam, color = "Full BBslope"), show.legend = F, size = 0.5) +
  geom_line(data = qet.d.long.sma %>% subset(par.sam == 7012), aes(group = par.sam, color = "Half BBslope"), show.legend = F, size = 0.5) +
  geom_line(aes(x= date, y = sp, color = "Observed"), size = 0.5) +  
  theme(legend.position = "top") +
  scale_colour_manual(name = "", values = col2) +
  ylab("ET [mm/day]") +
...
p.et1.m.p <- ggplot(qet.m.long.sub, aes(x = date, y = value)) +
  # geom_line(data = qet.m.long.sub %>% subset(par.sam == 7015), aes(group = par.sam, color = "Half Vcmax"), show.legend = F, size = 0.2) +
  # geom_line(data = qet.m.long.sub %>% subset(par.sam == 7011), aes(group = par.sam, color = "Half HKsatADJ"), show.legend = F, size = 0.2) +
  geom_line(data = qet.m.long.sub %>% subset(par.sam == 7013), aes(group = par.sam, color = "Full BBslope"), show.legend = F, size = 0.2) +
  geom_line(data = qet.m.long.sub %>% subset(par.sam == 7012), aes(group = par.sam, color = "Half BBslope"), show.legend = F, size = 0.2) +
  geom_line(data = qet.m.long.sub %>% subset(par.sam == 7014), aes(group = par.sam, color = "Half BBslope+ Half Vcmax"), show.legend = F, size = 0.2) +
  geom_path(aes(x= date, y = obs, color = "Observed"), size = 0.2, linetype = 5) +  
  geom_point(aes(x= date, y = obs, color = "Observed"), size = 1, shape = 1) +  
  theme(legend.position = "top") +
  scale_colour_manual(name = "", values = col2) +
  ylab("ET [mm/day]") +
  xlab("Date") + 
  scale_x_date(date_breaks = "1 year", labels = function(x) format(x, "%b%y"))
ggsave(paste0("ET_Obs_vs_model_monthly_all_years_points_lines.jpeg"), plot = p.et1.m.p, path = file.path("figures", current.folder), device = "jpeg", height = 3, width = 15, units='in')


p.et.m.p.halfbb.halfvc <-  p.et1.m.p +
  geom_line(data = qet.m.long.sub %>% subset(par.sam == 7014), aes(group = par.sam, color = "Half BBslope+ Half Vcmax"), show.legend = F, size = 0.2)
ggsave(paste0("ET_Obs_vs_model_monthly_all_years_points_lines_halfbb.halfvc.jpeg"), plot = p.et.m.p.halfbb.halfvc, path = file.path("figures", current.folder), device = "jpeg", height = 3, width = 15, units='in')
...
df.factor <- 1
set.order <- 21 ## how many days to average over
gpp.sma.sp <- smooth::sma(obs.gpp.d$obs, order = set.order)

obs.gpp.d$sp <- as.numeric(gpp.sma.sp$fitted) 
...
gpp.d.long <- gather(gpp.d, key = "par.sam", "value", -date, -obs, -sp) %>%
  mutate(par.sam = as.numeric(par.sam))

gpp.sma <- gpp.d.sub 
for (i in 1:nsam){
  sma.i <- smooth::sma(gpp.d.sub[, i + 3], order = set.order)
...
gpp.d.long.sma <- gather(gpp.sma, key = "par.sam", "value", -date, -obs, -sp) %>%
  subset(date >= as.Date("2012-07-01") & date < as.Date("2017-08-31")) %>% droplevels() 


p.gpp.d <- ggplot(gpp.d.long.sma,
       aes(x = date, y = value)) +
  # geom_line(data = gpp.d.long.sma %>% subset(par.sam == 7015), aes(group = par.sam, color = "Half Vcmax"), show.legend = F, size = 0.5) +
  # geom_line(data = gpp.d.long.sma %>% subset(par.sam == 7011), aes(group = par.sam, color = "Half HKsatADJ"), show.legend = F, size = 0.5) +
  geom_line(data = gpp.d.long.sma %>% subset(par.sam == 7014), aes(group = par.sam, color = "Half BBslope+ Half Vcmax"), show.legend = F, size = 0.5) +
  geom_line(data = gpp.d.long.sma %>% subset(par.sam == 7013), aes(group = par.sam, color = "Full BBslope"), show.legend = F, size = 0.5) +
  geom_line(data = gpp.d.long.sma %>% subset(par.sam == 7012), aes(group = par.sam, color = "Half BBslope"), show.legend = F, size = 0.5) +
  scale_colour_manual(name = "", values = col2) +
  geom_line(aes(y = sp, colour = "Observed"), size = 0.5) +
  ylab("GPP [gC/m^2/d]") +
  xlab("Date") + 
  theme(axis.text = element_text(size = 14, face = "plain"),
        legend.text = element_text(size = 16, face = "plain"),
...
g1 <- ggplot(swc.d.long.sub.mini, aes(x = date, y = value)) +
  geom_line(aes(x= date, y = obs, color = "Observed"), size = 0.5) +
  geom_line(data = swc.d.long.sub.mini %>% subset(par.sam == 7011), aes(group = par.sam, color = "Half HKsatADJ"), show.legend = F, size = 0.5) +
  geom_line(data = swc.d.long.sub.mini %>% subset(par.sam == 7014), aes(group = par.sam, color = "Half BBslope+ Half Vcmax"), show.legend = F, size = 0.5) +
  geom_line(data = swc.d.long.sub.mini %>% subset(par.sam == 7013), aes(group = par.sam, color = "Full BBslope"), show.legend = F, size = 0.5) +
  geom_line(data = swc.d.long.sub.mini %>% subset(par.sam == 7012), aes(group = par.sam, color = "Half BBslope"), show.legend = F, size = 0.5) +
  theme(legend.position = "top") +
