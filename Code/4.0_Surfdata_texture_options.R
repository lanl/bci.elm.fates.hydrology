...
# ORGANIC:units = "kg/m3 (assumed carbon content 0.58 gC per gORGANIC)" ;
...
### ORGANIC
###----
# Grimm, R., Behrens, T., Marker, M., and Elsenbeer, Helmut. 2008. "Soil organic carbon concentrations and stocks on Barro Colorado Island -- Digital soil mapping using RandORGANIC Forests analysis." Geoderma. 146 (1-2):102–113. 
...
# In order to cORGANICpare lateral and vertical SOC distribution patterns 
# we cORGANICputed the SOC stocks (Fig. 6a–d), since natural pedons include non-soil cORGANICponents 
...
#  This is due to their cORGANICparatively high bulk density and low stoniness. 
# ORGANIC <- data.frame(depth = c(5, 15, 25, 40), 
...
# GranulORGANICetry % of fine earth
# Sample depth      0-6	  6-16	36 - 46	    66 - 76	      136 - 146	  156 - 166
...
ORGANIC <- data.frame(depth = c(0.007, 0.03, 0.062, 0.11, 0.21, 0.36, 0.62, 1.04, 1.73, 2.86),
...
ORGANIC$ORGANIC <- round(ORGANIC$stock.soc/0.58, 1) #in kg/m3
ORGANIC
# depth stock.soc PCT_CLAY PCT_SAND   ORGANIC
...
# FrORGANIC: data-raw/BCI 50 ha plot soil texture_Matteo.xls
# 
...
ORGANIC$ORGANIC
# 57 31 23 38 # kg/m3
# In ORGANIC after the first seven values (0-62 cm depth), tsking ORGANIC as approximately 10, and then 1, whose value is used for the depths > 3 m
...
## frORGANIC the BCI texture data above, can estimate texture for fsurdat depths:
txr.2 <- data.frame(depth = c(0.007, 0.03, 0.062, 0.11, 0.21, 0.36, 0.62, 1.04, 1.73, 2.86),
           ...
           #ORGANIC = c(57, 57, 57, 44, 31, 23, 38, 30, 10, 1))
           ORGANIC = c(12.1, 12.1, 12.1, 4.6, 3.2, 1.8, 0, 0, 0, 0))

## if only texture data frORGANIC the North Location is used, instead of an average across the four locations:
...
txr.4.15depths <- data.frame(depth = as.numeric(nc$var[['H2OSOI']]$dim[[2]]$vals),
                             PCT_SAND = c(txr.4$PCT_SAND, rep(txr.4$PCT_SAND[length(txr.4$PCT_SAND)], 5)),
                             PCT_CLAY = c(txr.4$PCT_CLAY, rep(txr.4$PCT_CLAY[length(txr.4$PCT_CLAY)], 5)),
                             ORGANIC = c(txr.4$ORGANIC, rep(txr.4$ORGANIC[length(txr.4$ORGANIC)], 5)))
write.csv(txr.4.15depths, file = file.path("results", current.folder, "AVA_soil_texture_in_CLM_15_depths.csv"), row.names = FALSE)
### default values:
# txr.1 <-
# PCT_SAND =
#   // PCT_SAND(0,0)
